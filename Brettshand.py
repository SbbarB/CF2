...
class Hand:
    ...
    def update(self):
        ...
        if self.current_round == 0:
            ...
            if len(self.active_players) > 1:  # Check if hand continues
...
class Dealer():
    ...
    def animate_hole_card(self, card):
        if card is not None:
            if hasattr(card, 'position') and card.position is not None:
                current_time = pygame.time.get_ticks()
    ...
    def deal_turn(self):
        turn_x = self.players_list[0].cards[0].card_surf.get_width() * 3 + (self.players_list[0].cards[0].card_surf.get_width() * 2 + 60)

        if self.can_deal and self.can_deal_flop and self.dealt_cards - (self.num_players * 2) < 4:
            self.flop.cards.append(self.deck[-1])
            self.flop.cards[self.current_flop_index].position = (turn_x, self.flop.cards[self.current_flop_index].card_y)
            self.deck.pop(-1)
            self.last_dealt_flop_time = pygame.time.get_ticks()
            self.can_deal_flop = False
            self.current_flop_index += 1
    ...
    def deal_river(self):
        river_x = self.players_list[0].cards[0].card_surf.get_width() * 4 + (self.players_list[0].cards[0].card_surf.get_width() * 3 + 80)

        if self.can_deal and self.can_deal_flop and self.dealt_cards - (self.num_players * 2) < 5:
            self.flop.cards.append(self.deck[-1])
            self.flop.cards[self.current_flop_index].position = (river_x, self.flop.cards[self.current_flop_index].card_y)
            self.deck.pop(-1)
            self.last_dealt_flop_time = pygame.time.get_ticks()
            self.can_deal_flop = False
            self.current_flop_index += 1
    ...
    def update(self):
        ...
        if self.dealt_cards == (self.num_players * 2) + 4:
            self.deal_river()
